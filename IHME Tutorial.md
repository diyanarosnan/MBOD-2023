...
LOCATION : MALAYSIA
AGE      : < 1 YEAR, 12 - 23 MONTHS, 2-4 YEARS, 5-9 YEARS, 10-14 YEARS, 15-19 YEARS, 20-24 YEARS, 25-29 YEARS, 30-34 YEARS, 35-39 YEARS, 40-44 YEARS, 45-49 YEARS, 50-54 YEARS, 55-59 YEARS, 60-64 YEARS, 65-69 YEARS, 70-74 YEARS, 75-79 YEARS, 80-84 YEARS, 85+ YEARS
SEX      : FEMALE &  MALE
YEAR     : 2013 - 2021

INSERT POPULATION FROM IHME - 2013 UNTIL 2021
LOCATION : MALAYSIA
AGE      : < 1 YEAR, 12 - 23 MONTHS, 2-4 YEARS, 5-9 YEARS, 10-14 YEARS, 15-19 YEARS, 20-24 YEARS, 25-29 YEARS, 30-34 YEARS, 35-39 YEARS, 40-44 YEARS, 45-49 YEARS, 50-54 YEARS, 55-59 YEARS, 60-64 YEARS, 65-69 YEARS, 70-74 YEARS, 75-79 YEARS, 80-84 YEARS, 80+ YEARS
SEX      : FEMALE &  MALE
YEAR     : 2013 - 2021
...
```
number.female <- read.csv("C:/Users/DELL/Downloads/IHME-GBD_2021_ALZHEIMER_FM_DATA_NUMBER/IHME-GBD_2021_DATA-9f622dd7-1.csv", header = TRUE) %>% # CHANGE TO YOUR DIRECTORY ACCORDINGLY
  filter(sex_name == "Female", year %in% years) %>% # FILTER WILL ONLY CHOOSE FEMALE AND YEARS THAT WE WANT TO INCLUDE
  select(age_name, year, val) %>% # SELECT ONLY THE COLUMN WE ARE INTERESTED IN
  pivot_wider(names_from = year, values_from = val) %>% # CHANGE THE TABULATION TO BECOME WIDER
  add_row(age_name = c("<5 years", "5-14 years", "15-29 years", "30-44 years", "45-59 years", "60-69 years", "70-79 years", "80+ years")) # ADD NEW ROWS
```
...
```
rate.female <- data.frame(age_name = custom_order, "2013" = NA, "2014" = NA, "2015" = NA, "2016" = NA, "2017" = NA, "2018" = NA, "2019" = NA)
...
GENERATE AN EMPTY YLD DATAFRAME
```
